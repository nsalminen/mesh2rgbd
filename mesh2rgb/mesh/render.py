...
Only use rasterization rendering is used here.
Note that:
...
def rasterize_triangles(vertices: np.ndarray, triangles: np.ndarray, h: int, w: int):
    """
    Args:
        vertices: [nver, 3]
        triangles: [ntri, 3]
    ...
    depth_buffer = np.zeros([h, w]) - np.iinfo(np.dtype("uint16")).max  # Set the initial z to the furthest position
    triangle_buffer = np.zeros([h, w],
                               dtype=np.int32) - 1  # If tri index = -1, the pixel has no triangle correspondance
...
def render_rgbd(vertices: np.ndarray, triangles: np.ndarray, colors: np.ndarray, triangle_mask: np.ndarray, h: int,
                w: int, BG: np.ndarray = None):
    """ render rgb image and depth image (RGBD)
    Args:
        vertices: [nver, 3]
        triangles: [ntri, 3]
        colors: [nver, 3]
    ...
    if BG is None:
        ...
        image = BG
    depth_buffer = np.zeros([h, w], dtype=np.float32, order='C') - np.iinfo(np.dtype("uint16")).max

    if triangle_mask is None:
        triangle_mask = np.ones(triangles.shape[0], dtype=int)

    #  Change orders. -> C-contiguous order(column major)
    vertices = vertices.astype(np.float32).copy()
    triangles = triangles.astype(np.int32).copy()
    colors = colors.astype(np.float32).copy()
    triangle_mask = triangle_mask.astype(np.int32).copy()
    ###
    st = time()
    mesh_core_cython.render_rgbd_core(
...
def render_texture(vertices: np.ndarray, triangles: np.ndarray, texture: np.ndarray, tex_coords: np.ndarray,
                   tex_triangles: np.ndarray, h: int, w: int, c: int = 3, mapping_type: str = 'nearest',
                   BG: np.ndarray = None):
