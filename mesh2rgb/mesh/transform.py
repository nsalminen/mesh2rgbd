...
Edited by Nels Numan (n.salminen@me.com)
'''
...
def lookat_camera(vertices, camera):
    """ 'look at' transformation: from world space to camera space. Camera eye is relative to object origin (set to 0,0,0).
    ...
    """
    if camera['at'] is None:
        at = np.array([0, 0, 0], np.float32)
    if camera['up'] is None:
    ...
    x_aixs = normalize(np.cross(camera['up'], z_aixs))  # look right
...
def perspective_project(vertices, fovy, aspect_ratio=1., near=0.1, far=1000.):
    ...
    '''
    fovy = np.deg2rad(fovy / 2)
    top = near * np.tan(fovy)
    bottom = -top
    right = top * aspect_ratio
    left = -right

    # -- Non-homogeneous, only fovy, as we don't want to transform our z-values
    projected_vertices_fov = vertices.copy()
    projected_vertices_fov[:, 0] = -(near / right) * vertices[:, 0] / vertices[:, 2]
    projected_vertices_fov[:, 1] = -(near / top) * vertices[:, 1] / vertices[:, 2]

    # projected_vertices_fov[projected_vertices_fov[:,2] > far] = [0, 0, 0]
    # projected_vertices_fov[projected_vertices_fov[:, 2] < near] = [0, 0, 0]
    ...
    # projected_vertices_homo = vertices_homo.dot(P.T)
    # projected_vertices_homo = projected_vertices_homo/projected_vertices_homo[:,3:]
    # projected_vertices_homo = projected_vertices_homo[:,:3]
    # projected_vertices_homo[:,2] = -projected_vertices_homo[:,2]

    return projected_vertices_fov
...
def world_to_image(vertices: np.ndarray, triangles: np.ndarray, colors: np.ndarray, obj, camera, w: int = 512,
                   h: int = 512):
    """ Transform vertices from world space -> camera space -> image space
    Args:
        vertices: [3, nver]
        triangles: [3, ntri]
        colors: [3, nver]
        obj: dict, object transformation parameters
        camera: dict, camera parameters
        h: height of rendering
        w: width of rendering

    Returns:
        image_vertices: [3, nver], vertices in image coordinate space
    """
    ...
    if camera['proj_type'] == 'orthographic':
        ...
        camera_vertices = mesh.transform.lookat_camera(transformed_vertices, camera)
        # pyrender_view(camera_vertices, colors)
        # Camera space to image space. (Projection) if orth project, omit
        projected_vertices = mesh.transform.perspective_project(camera_vertices, camera['fovy'], w / h,
                                                                near=camera['near'],
                                                                far=camera['far'])
        # To image coordinates (position in image)
